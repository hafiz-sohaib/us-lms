        ...
        if (response.notices.length > 0) {
            response.notices.map((notice, index) => {
                output += `<tr class="align-middle text-center">
                    ...
                    <td>
                        <button type="button" onclick="delete_notice('${notice._id}')" class="btn p-0"><i class="fa-solid fa-trash" style="font-size: 18px"></i></button>
                    </td>
                </tr>`;
            })
        }else{
...
function get_everyone_notices() {
    $('#display_everyone_notices').html(teacher_loader());

    $.get("/api/v1/notices?notice_roll=for-everyone", response => {
        let output = "";
        ...
        $('#display_everyone_notices').html(output);
    });
}
